...
--2. What is the number of nodes per region?
	...
	FROM regions		
	JOIN customer_nodes AS customers		
...
WITH avg_day_reallocation AS (
  SELECT
    region_id,
    customer_id,
    node_id,
    SUM(DATEDIFF(day, start_date, end_date)) AS avg_day
  FROM customer_nodes
  WHERE end_date != '9999-12-31'
...
, percentile_ranks AS (
  SELECT
    region_id,
    avg_day,
    NTILE(100) OVER (PARTITION BY region_id ORDER BY avg_day) AS percentile_rank
  FROM avg_day_reallocation
)
SELECT 
  region_id,
  MAX(CASE WHEN percentile_rank = 50 THEN avg_day END) AS median,
  MAX(CASE WHEN percentile_rank = 80 THEN avg_day END) AS percentile_80,
  MAX(CASE WHEN percentile_rank = 95 THEN avg_day END) AS percentile_95
...
SELECT 
  ...
  ROUND(AVG(total_amount), 2) AS avg_deposit_amt
FROM deposits;
...
WITH monthly_changes AS (
  SELECT 
    customer_id,
    EOMONTH(txn_date) AS last_day_of_month,
    SUM(CASE 
